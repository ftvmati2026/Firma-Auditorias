...
if (!docId) {
    ...
    // Buscar y Cargar documento desde la base de datos
...
btnClear.addEventListener('click', () => {
    ...
    document.getElementById('firmanteNombre').disabled = false;
    document.getElementById('firmanteID').disabled = false;
...
btnDone.addEventListener('click', () => {
    ...
    const firmanteNombre = document.getElementById('firmanteNombre');
    const firmanteID = document.getElementById('firmanteID');

    if (!chkConformidad.checked) {
        alert("Debe tildar la casilla de conformidad para poder continuar.");
        return;
    }
    if (firmanteNombre.value.trim() === '') {
        alert("Por favor, ingrese el Nombre y Apellido del firmante.");
        firmanteNombre.focus();
        return;
    }
    
    const valorIdLimpio = firmanteID.value.trim().replace(/\./g, '');
    if (valorIdLimpio === '') {
    ...
    if (dniDetectadosEnPDF.length > 0) {
        if (!dniDetectadosEnPDF.includes(valorIdLimpio)) {
            alert(`SISTEMA DE SEGURIDAD:\n\nEl ID ingresado (${valorIdLimpio}) no coincide con ninguno de los registrados en el documento.\n\nPor favor, revise estar colocando bien sus datos.`);
            firmanteID.focus();
            return;
        }
    }
    ...
    firmanteNombre.disabled = true;
    firmanteID.disabled = true;
    signaturePad.off();
...
btnSubmit.addEventListener('click', async () => {
    try {
        ...
        await db.collection('auditorias').doc(docId).update({
            ...
            firmanteID: document.getElementById('firmanteID').value.trim(),
            conformidad: true,
